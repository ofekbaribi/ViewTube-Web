...
const UserProfile = () => {
  ...
  const { getUserLastName } = useUser();
  const decodedUsername = decodeURIComponent(username);

  // Get videos uploaded by the user
  const userVideos = getVideosByUsername(decodedUsername);
  const profileImage = getProfilePicture(decodedUsername);
  const firstName = getUserFirstName(decodedUsername);
  const lastName = getUserLastName(decodedUsername);
